...
impl RipPacket {
    ...
    pub fn from_bytes(data: &[u8]) -> RustRouteResult<Self> {
        if data.len() < 4 {
            return Err(crate::RustRouteError::ProtocolError("Packet too short".to_string()));
        }

        let command = match data[0] {
            1 => RipCommand::Request,
            2 => RipCommand::Response,
            _ => return Err(crate::RustRouteError::ProtocolError("Invalid command".to_string())),
        };
    ...
    pub fn new_update(_router_id: uuid::Uuid, routes: Vec<crate::routing_table::Route>) -> Self {
        let entries = routes.into_iter().map(|route| RipEntry {
            address_family: 2, // IP
            route_tag: 0,
    ...
    pub fn to_json(&self) -> crate::RustRouteResult<String> {
        serde_json::to_string(self)
            .map_err(|e| crate::RustRouteError::ProtocolError(format!("JSON serialization failed: {}", e)))
    }

    /// Deserialize packet from JSON
    pub fn from_json(json_str: &str) -> crate::RustRouteResult<Self> {
        serde_json::from_str(json_str)
            .map_err(|e| crate::RustRouteError::ProtocolError(format!("JSON deserialization failed: {}", e)))
    }

    /// Validate packet contents
    pub fn validate(&self) -> crate::RustRouteResult<()> {
        if self.version != 2 {
            return Err(crate::RustRouteError::ProtocolError("Invalid RIP version".to_string()));
        }
        
        for entry in &self.entries {
            if entry.metric > 16 {
                return Err(crate::RustRouteError::ProtocolError("Invalid metric value".to_string()));
            }
        }
